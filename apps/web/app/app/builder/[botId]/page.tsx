...
import { getBot, updateBotGraph, updateBotMeta, saveBotVersion } from '@/lib/bots'
...
export default function BuilderPage() {
  ...
  const { saveGraph: saveLocalGraph, updateBot: updateLocalBot, savePreset, snapshotVersion } = useWorkspace()
  ...
  /* Fetch the bot from Postgres on mount */
  useEffect(() => {
    async function fetchBot() {
      try {
        ...
        console.error('Failed to load bot from DB:', err)
        if (localBot) setBot(localBot)
      } finally {
        setLoading(false)
      }
    }
  ...
  const save = useCallback(
    async (silent = false) => {
      ...
      try {
        await updateBotGraph(bot.id, graphData)
      } catch (err) {
        console.error('Failed to save to Postgres:', err)
      }
      saveLocalGraph(bot.id, graphData)
      markSaved()
      if (!silent) {
        pushLog('info', 'Graph saved to database.')
        toast.success('Bot saved', `${nodes.length} nodes, ${edges.length} connections.`)
      }
    },
    [bot, nodes, edges, notes, frames, saveLocalGraph, markSaved, pushLog],
  )
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        console.error('Autosave to Postgres failed:', err)
      }
      saveLocalGraph(bot.id, graphData)
      markSaved()
  ...
  }, [dirty, bot, nodes, edges, notes, frames, saveLocalGraph, markSaved, pushLog])
  ...
  return (
    <div className="flex h-[calc(100svh-3.5rem)] flex-col">
      <BuilderToolbar
        ...
        onRename={async (name) => {
          try {
            await updateBotMeta(bot.id, { name })
            await saveBotVersion(bot.id, `Renamed to ${name}`)
          } catch (e) {
            console.error('Rename failed:', e)
          }
          updateLocalBot(bot.id, { name })
