...
class ProviderOrchestrator:
    ...
    def _build_provider_metadata(self) -> dict[str, ProviderMetadata]:
        ...
        return {
            ...
            'groq': ProviderMetadata(
                ...
                config=self.config.get_model_config(self.config.GROQ_MODEL),
                supports_chart=False,
            )
        }
    ...
    async def invoke(
        ...
        *,
        ...
        if provider == "groq":
            return await self._invoke_groq(metadata, messages, effective_model, chart, chart_image)
        return InvocationResult(
            success=False,
    ...
    async def get_text_response(
        ...
        if effective_provider == "all":
            result = await self.invoke_with_fallback(
                ["googleai", "local", "openrouter", "blockrun", "groq"], messages, model=model
            )
            return result
    ...
    def _log_failure(self, provider: str) -> None:
        ...
        if provider == "googleai":
            ...
            self.logger.warning("Groq failed or rate limited.")
    ...
    def _log_unavailable_guidance(self, provider: str) -> None:
        ...
        if not metadata or not metadata.client:
            if provider == "openrouter":
                ...
                self.logger.error("Groq client not initialized. Check GROQ_API_KEY in keys.env")
        elif provider == "local":
