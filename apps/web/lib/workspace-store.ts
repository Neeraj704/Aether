  ...
  persist(
    ...
    }),
    {
        ...
        return {
          getItem: (key: string) => {
            try {
              ...
              console.warn('[Workspace Store] Read error:', err)
              return null
            }
          },
          setItem: (key: string, value: string) => {
            try {
              ...
              console.warn('[Workspace Store] LocalStorage full, clearing heavy cache:', err)
              try {
                // If quota is exceeded, prune runs or reset heavy keys
                const parsed = JSON.parse(value)
                if (parsed?.state?.runs) {
                  parsed.state.runs = parsed.state.runs.slice(0, 2).map((r: any) => ({
                    ...r,
                    trades: (r.trades || []).slice(0, 10),
                    equity: (r.equity || []).slice(0, 50),
                  }))
                  localStorage.setItem(key, JSON.stringify(parsed))
                }
              } catch (fallbackErr) {
                console.warn('[Workspace Store] Could not save compressed state:', fallbackErr)
              }
            }
          },
          removeItem: (key: string) => {
            try {
              ...
              console.warn('[Workspace Store] Remove error:', err)
            }
          },
        } as any
      }),
      ...
      partialize: ({ hydrated, ...rest }) => ({
        ...rest,
        // Only store the latest 3 runs with capped trades to stay well under 5MB
        runs: (rest.runs || []).slice(0, 4).map((r) => ({
          ...r,
          trades: (r.trades || []).slice(0, 30).map((t) => ({
            ...t,
            executionFlow: undefined, // loaded on-demand or stored in-memory
          })),
          equity: (r.equity || []).slice(0, 100),
        })),
        activity: (rest.activity || []).slice(0, 20),
        notifications: (rest.notifications || []).slice(0, 20),
      }),
