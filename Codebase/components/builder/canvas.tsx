...
/** Lane guide painted behind the graph so layer order stays legible. */
function LaneGuides() {
        ...
        return (
          ...
          >
            ...
            <button
              ...
              className={cn(
                'pointer-events-auto absolute top-3 left-4 flex items-center gap-2 rounded-[var(--radius-pill)] border border-border/70 px-2.5 py-1 text-left backdrop-blur-md transition-colors',
                collapsed ? 'bg-secondary' : 'bg-background/55 hover:bg-secondary',
              )}
            >
              <span
...
function FlowCanvas({
    ...
    (changes: NodeChange[]) => {
      ...
      if (selChange) {
        const picked = changes
          .filter((c): c is NodeChange & { type: 'select'; id: string; selected: boolean } => c.type === 'select')
          .filter((c) => c.selected)
          .map((c) => c.id)
        setSelection(picked)
      }
  ...
  const onDrop = useCallback(
    (event: React.DragEvent) => {
      ...
      const componentId = event.dataTransfer.getData('application/aether-component')
      ...
      >
        ...
        {view.minimap ? (
          <MiniMap
            ...
            className="!bottom-16 !right-4 !m-0 overflow-hidden !rounded-[12px] !border !border-border !bg-background/80 !backdrop-blur-xl"
