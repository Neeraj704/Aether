    ...
    <header className="glass-chrome sticky top-0 z-40 flex h-14 items-center gap-3 px-4 lg:px-6">
      ...
      <div className="flex flex-1 items-center justify-end gap-1.5 sm:gap-2">
        ...
        <PillButton
          ...
          onClick={async () => {
            try {
              const { createBot: createBotDB } = await import('@/lib/bots')
              const bot = await createBotDB({ name: 'Untitled Bot' })
              router.push(`/app/builder/${bot.id}`)
              toast.success('New bot created', 'Start with a data feed from Layer I.')
            } catch (err: any) {
              toast.error('Failed to create bot', err?.message || 'Could not persist bot to database')
            }
          }}
        >
