...
class TemplateManager:
    ...
    def build_response_template(self, has_chart_analysis: bool = False,
        ...
        if verbosity == "high":
            ...
            _output_header = (
                "Output: JSON block FIRST (required), then 3 brief plain-text lines. JSON is truth. No markdown headings. No preamble before JSON."
            )
            _narrative_section = (
                f"After JSON:\n"
                f"1) BIAS: Bearish/Bullish/Neutral\n"
                f"2) KEY LEVEL: nearest support or resistance\n"
