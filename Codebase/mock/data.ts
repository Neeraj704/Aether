...
export function generateBacktest(params: {
  ...
  const trades: Trade[] = Array.from({ length: Math.min(tradeCount, 60) }, (_, i) => {
    ...
    return {
      ...
      side: (rand() > 0.38 ? 'long' : 'short') as Trade['side'],
      entryTime: entry.toISOString(),
      exitTime: new Date(entry.getTime() + holdMs).toISOString(),
      size,
      pnl,
      pnlPct: Number(((pnl / size) * 100).toFixed(2)),
      triggerNode: TRIGGER_NODES[Math.floor(rand() * TRIGGER_NODES.length)],
      confidence: Number((0.52 + rand() * 0.44).toFixed(2)),
    }
